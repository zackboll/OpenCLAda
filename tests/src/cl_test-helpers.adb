...
package body CL_Test.Helpers is
   ...
   begin
      IO.Put_Line (Item => 40 * '-');
      IO.Put_Line (Item => "Error callback invoked:");
      IO.Put_Line (Item => "Info String: """ & Error_Info & """");
      IO.Put      (Item => "Binary data: (");
   ...
   begin
      while not IO.End_Of_File (File => File) loop
         Append (Source => Contents, New_Item => IO.Get_Line (File => File));
